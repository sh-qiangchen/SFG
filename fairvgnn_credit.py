...
def run(data, args):
    ...
    if(args.encoder == 'MLP'):
        ...
        optimizer_e = torch.optim.Adam([dict(params=encoder.parameters(), weight_decay=args.e_wd)], lr=args.e_lr)  
    ...
    for count in pbar:      
        ...
        for epoch in range(0, args.epochs):
            ...
            ResLogFile1 = os.path.join('mask-logs', args.dataset) + "_ResLog1-ct.txt"
            with open(ResLogFile1, "a+") as file:
                now = time.localtime()
                formatted_time = time.strftime("%Y-%m-%d %H:%M:%S ", now)
                strLog = formatted_time + "weight1: {:.4f} weight2:{:.4f} weight3:{:.4f}".format(encoder.gcn_stack[1].lin_r.weight.detach().data[7,7].cpu().detach().numpy(), \
                    encoder.gcn_stack[1].lin_r.weight.detach().data[9,9].cpu().detach().numpy(),\
                        encoder.gcn_stack[1].lin_r.weight.detach().data[13,13].cpu().detach().numpy() ) 
                file.write(strLog +"\n")
                
            ResLogFile1 = os.path.join('mask-logs', args.dataset) + "_ResLog_mask-ori.txt"
            with open(ResLogFile1, "a+") as file:
                now = time.localtime()
                formatted_time = time.strftime("%Y-%m-%d %H:%M:%S ", now)
            ...
            # # normalized_representations = F.normalize(h, p=2, dim=1)
            # # cosine_similarity_matrix = torch.mm(normalized_representations, normalized_representations.t())
            # import seaborn as sns
            # # sns.heatmap(cosine_similarity_matrix[50:60, 50:60].cpu().detach().numpy(), annot=True, cmap='YlGnBu') 
            ...
            if cur_val_tradeoff <= best_val_tradeoff:
                ...
                # cosine_similarity_matrix = torch.mm(normalized_representations, normalized_representations.t())
                # import seaborn as sns
                # sns.heatmap(torch.clone(encoder.gcn_stack[1].lin_r.weight.detach().data)[4:12,4:12].cpu().detach().numpy(), annot=True, cmap='YlGnBu')  # 如果需要显示具体数值，将annot设置为True
                # # sns.heatmap(cosine_similarity_matrix[50:60, 50:60].cpu().detach().numpy(), annot=True, cmap='YlGnBu')  # 如果需要显示具体数值，将annot设置为True
