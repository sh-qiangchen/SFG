...
def run(data, args):
    ...
    for count in pbar:
        ...
        for epoch in range(0, args.epochs):
            ...
            ResLogFile1 = os.path.join('mask-logs', args.dataset) + "_ResLog_mask.txt"
            with open(ResLogFile1, "a+") as file:
                now = time.localtime()
                formatted_time = time.strftime("%Y-%m-%d %H:%M:%S ", now)
                strLog = formatted_time + "weight1: {:.4f} weight2:{:.4f} weight3:{:.4f} weight4:{:.4f}".format(generator.weights[0, 0], \
                    generator.weights[1, 0], generator.weights[2, 0], generator.weights[4, 0]) 
                file.write(strLog +"\n")  
            ...
            # sns.heatmap(encoder.gcn_stack[1].lin_l.weight.detach().data[3:13,3:13].cpu().detach().numpy(), annot=True, cmap='YlGnBu', fmt='.2f') 
            # plt.savefig("./maps/2-german-ct-heatmap-" + str(epoch) + ".png")
            # # plt.show()
            # plt.clf()  # 清除当前图形
